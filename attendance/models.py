...
class ClassSchedule(models.Model):
    ...
    def clean(self):
        if self.subject.teacher_id != self.teacher_id:
            raise ValidationError("Selected teacher must match the subject's assigned teacher.")
        if self.start_time >= self.end_time:
            raise ValidationError("Start time must be earlier than end time.")

    def __str__(self):
        return f"{self.subject.code} | {self.day_of_week} | {self.start_time} - {self.end_time}"
